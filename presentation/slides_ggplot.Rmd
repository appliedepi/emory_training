...
* Neale Batra  
 - Founder of Epi R Handbook. Previously @ Philly and Santa Clara County health departments. Currently working for WHO Geneva (COVID-19 analytics team)  
* Alex Spina  
...
4) Modify relevant "scales", such as a color scale  

--

5) Add "theme" design elements such as axis labels, title, fonts, sizes, color schemes, legends, or axes rotation  
...
*Note the indentation and parentheses*  
...
Scale commands control *how* the aesthetic mapping manifests, for example:  
* *Which* colors or shapes show for certain values    
...
Inside the command, give arguments such as `breaks=`, `values=`, or `labels=` (there are many possibilities).  
...
Here, the fill of a bar plot uses the default colors and y-axis breaks:  
...
First, in a `scale_fill_manual()` we list the color assignments in a vector (within `c()`).   
...
Use `lab()` to add title, subtitle, caption, axes labels, and to edit legend title  
...
```{r, eval=F, echo=T}
caption = stringr::str_glue("Data as of {Sys.Date()}"))
```

```{r, eval=F, echo=T}
caption = stringr::str_glue("{nrow(linelist %>% filter(is.na(date_onset)))} cases missing date of onset and not shown"))
```

**Legend title**  
In `labs()`, write the aesthetic that created the legend (e.g. `fill=`, `color=`, `shape=`) and provide the title in quotation marks.  
