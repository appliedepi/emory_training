...
XXX we could. Although I think that if we are short on time this slide can be compressed. Our main point is that there are other options for graphs in R and we focus on one. Maybe it's good to explain why? For me, good reasons are:
- very good for fast data exploration, especially of multidimensional data
- yet can produce very high quality final outputs
- many packages to extand functionalities
- a well structure grammar => high consistency
Do we add that to the slide?
...
???
...
As you can see, this is only a blank canvas, we have not defined what should be in the x and y axes, nor linked any data to be plot. It's an empty page waiting for you.


---
# Telling ggplot which data to use

.pull-left[

If a single dataframe is used for the plot, one can refer to it in `ggplot()`
...
Notes: if several dataframes are needed, they can be added in their own *geoms* (example later).
...
???
This does not produce visible changes, but ggplot will now look for columns within the _linelist_ dataframe.
...
**Mapping** is telling ggplot which variable goes into which _aesthetics_.
...
Geoms express the data as _geometry_ (lines, points, boxplots, histograms, etc.)

**ggplot2** propose many classical “geoms”:  
