...
The [Epi R Handbook](https://epirhandbook.com/index.html) contains more examples and more complex plots, for your personal reading  
...
* is good for fast data exploration, especially of multi-dimensional data
...
??? 
Should we explicit why consistency is good (the other points being self-explanatory in my opinion)? For me it leads to faster rates of learning: despite ggplot syntax being somewhat obscure for someone who never read a tutorial on it. It means that one you have grasped the basics of the basics, you can learn new geoms extremely fast. Also mix and match geoms in graphs. Also, better retention => less time spend searching the help page/internet because it always work in the same way.
...
Today we begin with a clean dataset in order to focus on visualization techniques.  
...
See the R markdown script from the previous session for the cleaning code. 
...
???
We will not linger on data cleaning today, in the interest of time.  
We will touch upon data structure and pivoting later in the presentation.  
...
- short style very dense (harder to read for some). Shorter scripts, but so what? The number of lines of your code is not an informative metrics.
...
* Mappings written in the initial `ggplot()` call apply to all following instructions (e.g. axes), *except instructed otherwise*  
* Mappings written within one geom apply only to it  

```{r, echo = TRUE, eval = FALSE, tidy = FALSE}
ggplot(data = linelist,     # open the plot andset dataset 
...
???
- Explain why you might use one or the other  
Or do we use an exemple later in the presentation to illustrate it?
...
But... this plot is difficult to interpret because points may be overlapping.  
...
We can add additional geoms to the current plot.

Geoms are added to the graph **in the order they are written in**: the smoothed line appears on top of the points.
...
For color & fill, use these names, or hex code...  


![](https://github.com/appliedepi/emory_training/blob/master/presentation/images/ggplot_colors_names.png?raw=true)  
...
![](https://github.com/appliedepi/emory_training/blob/master/presentation/images/points_symbols.png?raw=true)  
...
An aesthetic is *static* if it applies to all data points in the geom or in the graph.  

Static aesthetics are defined *outside* `aes()` to a **number or character value**.   
...
An aesthetis is *dynamic* if it varies between data points, based on the values of another column/variable.  

Dynamic aesthetics are defined *inside* the `aes()` and are assigned to a **colum name**.  
...
We can mix static and dynamic aesthetics:  
...
Above, `size = 7` and `alpha = 0.7`are static aesthetics defined outside any `aes()` and therefore not mapped to any column.  
...
Aesthetic mappings like `fill=` and `color=` create "groups" in the plot. Problems arise if your data are "grouped", but your plot does not account for this. In other words: you have multiple "y" values per x-value.  

Below, `outcome_time_data` has one row per week-outcome, but the plotting command does not account for this.  
...
This version includes `color=` mapped to the `died` column. So the lines separate nicely.  
...
---
# Epicurve - missing values 

.pull-left[

Here, "Unknown" is a distinct value. To add `NA` (missing) values as their own group, use `na_as_group=TRUE`  
...
weekly <- incidence(
  ...
  interval = "weeks",      # bin interval
  groups = eth_race,
* na_as_group = TRUE)      # missing as group
```
...
labs(title = "COVID-19 cases in ZIP 30024")
```
...
```{r, echo=F, eval=T}
zip30024 <- incidence(
  x = linelist %>% filter(zip == 30024), # define dataset
  date_index = date_onset,   # date column
  interval = "Sunday weeks", # bin interval
  groups = eth_race) 
...
???
This again uses the argument to accept color palettes
...
Three `gender` values are detected, so the plot is not a traditional pyramid.  
...
Let's remove "Unknown" as an explicit category, by converting to `NA` and dropping the unused factor level:  
...
`tabyl()` from **janitor** makes pretty tabulations
...
.pull-left[
Now make the pyramid again, using the revised dataset:  
...
labs(
  subtitle = str_glue(
    ...
    min_date = format(min(linelist_2g$date_report, na.rm=T), "%B %d %Y"),
  
    max_date = format(max(linelist_2g$date_report, na.rm=T), "%B %d %Y")
  )
) 
```
...
labs(
  ...
  subtitle = str_glue(
    ...
    min_date = format(
      min(linelist_2g$date_report, na.rm=T),
      "%B %d %Y"),
  
    max_date = format(
      max(linelist_2g$date_report, na.rm=T),
      "%B %d %Y")
  ),
  x = "Percent of total",
  y = "Age group",
...
To generate one graph by race/ethnicity:  
...
linelist %>% 
  ...
  ggplot(
    mapping = aes(
      ...
      color = age_group))+
  geom_line()+
* gghighlight::gghighlight(age_group %in% "40-49")+
  theme(legend.position = "none")
...
**ggplot2** works best when the dataset is "long".  

IMAGE OF PIVOTING DATA

You have seen workarounds for aggregated data, such as `geom_col()` for bar plots. But for all other plots it is far easier to shape your data to conform to this structure prior to plotting.  

You should get familiar with `pivot_longer()` from **dyplr**. We cannot go in-depth in this presentation, but see the [Epi R Handbook pivoting page](https://epirhandbook.com/pivoting-data.html).  
