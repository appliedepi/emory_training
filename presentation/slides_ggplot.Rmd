...
To add groups, specify them to `groups=` in the `incidence()` command AND `fill=` in the `plot()` command:  
...
Add `NA` (missing) values as their own group with `na_as_group=TRUE`  
...
Adjust legend title with `labs()`, as in any ggplot
```{r , echo = TRUE, eval = FALSE, tidy = FALSE}
plot(SunWeeks,
  fill = eth_race)+
*labs(fill = "Race and/nEthnicity")
```
...
Add other ggplot commands with `+`:  
...
Do not add `scale_x_date()` to modify. Instead, use custom arguments in `plot()`:  
...
See the Epi R Handbook [page on demographic pyramids](https://epirhandbook.com/demographic-pyramids-and-likert-scales.html) for more information on both.  
