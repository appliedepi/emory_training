...
* Mathilde Mousset  
 - With Epicentre (MSF). Former specialization in ecology.  
...
See the R markdown script from the previous session for all the cleaning code.  
...
Note above we pipe the adjusted data frame into `ggplot()` 
] 
...
linelist %>% 
  ...
  ggplot(
    mapping = aes(
      ...
      color = zip))+
  geom_line()+
* gghighlight::gghighlight(zip == "30004")+
  theme(legend.position = "none") # too many zip codes to show
...
---
# **scales** package - simplify date axis  

.pull-left[

`scales::label_date_short()` instantly makes easy date labels, and reduces the number of times year appears.
...
---
# **scales** package - percents axis  

.pull-left[

`scales::percent()` converts decimal proportions into percents. Note that if you converted to percent in advance the values would be characters and would not plot correctly.  
...
Note use of `scales::comma()` in the `str_glue()` labels to add commas.  
