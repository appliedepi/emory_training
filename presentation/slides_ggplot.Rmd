...
* Advanced tips such as using **gghighlight**, **scales** and **ggplotly**  
...
Today's work is conducted within a dedicated R project, which can be [downloaded as a zipped file](https://minhaskamal.github.io/DownGit/#/home?url=https://github.com/appliedepi/emory_training/tree/master/case_study):  
...
Today we begin with the cleaned linelist in order to focus on visualization techniques. See the R markdown script from the previous session for all the cleaning code.  
...
.footnote[The `import()` function can import almost any kind of file. An `.rds` file retains column classes from an R data frame.]
...
An aesthetic is *static* if it applies the same display to all data points in the geom or in the graph. Static aesthetics are defined *outside* `aes()` to a *number or character value*.  
...
An aesthetic is *dynamic*, if it varies between data points, based on the values in a column. Dynamic aesthetics are defined *inside* `aes()` and are assigned to a **column name**.  
...
We can  mix static and dynamic aesthetics. Above, `size = 7` and `alpha = 0.7` are static aesthetics defined outside any `aes()` and therefore not mapped to any column.  
]
...
Aesthetic mappings like `fill=` and `color=` create "groups" in the plot. Problems arise if you summarise your data "by group" but plot indiscriminately - despite having multiple y values per x value.  
...
.footnote[
Alert your audience if you free an axis! 
...
Plots appear sequentially by alphabet/factor level. Facet titles are combined.  
...
**Show two variables in a grid: `facet_grid()`.   
...
*Note the labels along the sides.*
...
**Dropping levels you don't want**
...
**Dropping empty levels or not?**

By default, `facet_grid` and `facet_wrap` drop unused levels/categories. 

Change `drop=` to `FALSE` to keep the empty facet
]
...
linelist %>% 
  ...
  ggplot(
    mapping = aes(
      ...
      color = age_group)) +
  geom_line() +
* gghighlight::gghighlight(age_group %in% 
                             c("40-49", "60-69)"))+
  theme(legend.position = "none")
...
```{r, eval=T, echo=T}
str_glue("{missing} of cases missing either age or gender are not shown.",
        missing = epikit::fmt_count(linelist, is.na(gender) | is.na(age_group))
        )
```
...
```{r, eval=T, echo=T}
str_glue("Fulton County, GA, reported {min_date} - {max_date}",
...
Create the first plot, assigning it a name:  
...
.footnote[
Source: ![Mike Bostock]("https://observablehq.com/@d3/projection-transitions")
]
