...
[1] www.epiRhandbook.com  

---
# EDGE  

Today's session will be *explanation* and *demonstration*. It follows from the previous [demonstration on data management and workflow](https://appliedepi.github.io/emory_training/presentation/slides.html).  
...
![The "EDGE" acronym for learning/teachings](https://github.com/appliedepi/emory_training/blob/master/presentation/images/EDGE%20flow.png?raw=true){height = 40%, width = 75%}
...
```{r}
fs::dir_tree(path = here("presentation"), recurse = FALSE)
```
...
linelist_clean <- import(here("presentation", "data", "linelist_cleaned.rds"))
```
...
#######################
hist(linelist_clean$age_years)  

# ggplot2 example
#######################################
# Sequence of weekly Monday dates for CENTRAL HOSPITAL
weekly_breaks <- seq.Date(
  from = floor_date(min(linelist_clean$date_onset, na.rm=T),   "week", week_start = 1), # monday before first case
  to   = ceiling_date(max(linelist_clean$date_onset, na.rm=T), "week", week_start = 1), # monday after last case
  by   = "week")

ggplot(linelist_clean) + 
  
  # histogram
  geom_histogram(
    mapping = aes(x = date_onset),
    
    breaks = weekly_breaks,   # pre-defined date vector - see top of ggplot section
  ...
  # labels and theme
  labs(
    title = "ggplot2 epicurve example",
  ...
  annotate(
    "rect",
    xmin  = as.Date(max(linelist_clean$date_onset, na.rm = T) - 21), # note must be wrapped in as.Date()
  ...
  annotate(
    "segment",
    x     = max(linelist_clean$date_onset, na.rm = T) - 21, # 21 days before last data
    xend  = max(linelist_clean$date_onset, na.rm = T) - 21, 
  ...
  annotate(
    "text",
    x = max(linelist_clean$date_onset, na.rm = T) - 15,
    y = 15,
...
############################################
# 
# plotly_box <- ggplot(data = linelist_clean)+
#   geom_boxplot(mapping = aes(y = age_years, x = outcome))+
#   theme_classic()
# 
# plotly::ggplotly(plotly_box)
```
...
**ggplot2** benefits from a wide variety of supplementary R packages that further enhance its functionality  
- **gganimate**  
- **ggthemr**  
- **ggdendro**  
- **gghighlight**  
...
[1] The “gg” in these names reflects the “grammar of graphics” used to construct the figures. 
