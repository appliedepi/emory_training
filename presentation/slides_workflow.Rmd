...
- Use `gtsummary`:
 ...
 + use **base R** versions of stats functions
...
linelist %>% 
  ...
  # add p-values from Chi-Sq test for gender
  add_p(gender ~ "chisq.test")
...
```{r, echo = FALSE}
linelist_raw %>% 
  filter(!is.na(died_covid), !is.na(case_gender)) %>% 
  mutate(outcome = ifelse(died_covid == "Yes", "Died", "Recovered")) %>% 
  select(outcome, gender = case_gender) %>% 
  tbl_summary(by = outcome) %>% 
  add_p(gender ~ "chisq.test") 
