...
1. Why R? 
2. Introduction to R
3. Setting the scene (project directory & Rmarkdown)  
4. Rmarkdown parametrisation  
5. File paths & importing data  
6. Data cleaning  
7. Merging data  
...
- **Functions** - a generalised operation
  + takes input(s)
  + returns output(s)
...
- **Packages**
  + A bundle of new functions!
  
.footer[

See the [EpiRHandbook section on R basics](https://epirhandbook.com/r-basics.html)
...
- **Characters** - chunks of text enclosed in "quotes"

--

- **Dataframes** - tables with named columns
...
- R has a rich set of packages to increase its functionality
...
- Or access functions within packages with `::`

```{r, eval = FALSE, echo = TRUE}
# get the first row of my table
table_first_row <- dplyr::slice(my_table, 1)
```
...
See the [EpiRHandbook section on R packages](https://epirhandbook.com/r-basics.html#packages)
...
- Piping passes an intermediate output to another function

 + do this **then** that 

--

- Access the pipe operator with `%>%`
...
clean_linelist <- linelist %>%
  # FIRST we discard some data
  discard_bad_data() %>% 
  # THEN we add an age group column
  # We want to use the age column to calculate these
  make_age_groups(col = age) 
```

.footnote[

See the [EpiRHandbook section on pipes](https://epirhandbook.com/r-basics.html?q=pipe#pipes)
...
1. Visually inspect data (`summary()`, `dplyr::glimpse()`)

--

2. Rename and choose fields (`dplyr::select()`)
