...
subtitle: "Data workflow with R: an overview and demonstration"
...
1. Setting the scene (project directory & Rmarkdown)  
2. Rmarkdown parametrisation  
3. File paths & importing data  
4. Data cleaning  
5. Merging data  
...
- Perform simple stats with `rstatix` or base R 

 + Base R usage: `t.test()`
 + `rstatix` usage: `t_test()`

--

- Examples:
 + `t_test()` - _T-test_
 + `shapiro_test()` - _Shapiro-Wilk test_
 + `kruskal_test()` - _Kruskal-Wallis/Mann-Whitney U test_
 + `chisq_test()` - _Chi-Squared test_
