...
rf_data <- linelist %>% 
  ...
  mutate(eth_race = fct_relevel(eth_race, "White, NH")) %>% 
...
shapefile <- st_transform(shapefile, crs = "+proj=longlat +datum=WGS84 +no_defs")
...
ggplot() + 
  ...
  geom_point(data = filter(linelist, lat > 1), 
...
shapefile <- left_join(shapefile, zip_counts, 
                       by = c("ZipCode" = "zip"))
shapefile <- shapefile %>% 
  mutate(incidence = age_categories(
    recent_inc,
    breakers = c(0, 1, 2, 3, 4, 5, 6, 100), 
    ceiling = TRUE
  ))
