...
For more details see: 
https://epirhandbook.com/cleaning-data-and-core-functions.html#column-names
https://epirhandbook.com/cleaning-data-and-core-functions.html#clean_tidyselect
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ -->
...
# it's a good habit to make sure the variables you're generating seem realistic (are you calculating them correctly?)
...
linelist <- linelist %>% 
     ...
     mutate(eth_race = case_when(
       ...
       # case_when() evaluates in order (like if, else if, else)
...
# produce a quick descriptive table of number of cases reported by date
# we aren't saving this, but just letting it display in the markdown
linelist %>% 
...
As you can see from above in Table \@ref(tab:timetab) we can see clear trends in reporting patterns - cases increase and peak in July 2020 and December/January 2020/2021
...
zip_counts <- left_join(zip_counts, 
  ...
  mutate(
    ...
    # replace 0s in incidence with 0
    across(contains("inc"), ~replace_na(.x, 0)),
...
# this can be done iteratively (we are repeating code)
...
total_cases <- purrr::map(
  ...
  ~tabyl(linelist, .x) %>%
    ...
    select("Characteristic" = .x, n_cases_total = n, perc_cases_total = percent)
) %>% 
...
recent_cases <- purrr::map(
  ...
  ~filter(linelist, 
    ...
    select(n_cases_recent = n, perc_cases_recent = percent)
) %>% 
  bind_rows()
...
{gtsummary}. We first create a the regression table, then a counts table and then 
finally merge the two and format the output. 
Note that a cox regression taking in to account for observation time might be a 
...
TODO: Demonstrate in-line functions from {gtsummary} 
...
TODO: gifs for maps projections to put in slides (consider just recreating w code)
https://twitter.com/mbostock/status/1292887409474994176
https://twitter.com/Amazing_Maps/status/456921227068968960/photo/1
...
# convert linelist data frame into sf object
...
map <- openmap(
  upperLeft = c(max(linelist_sf$lat, na.rm=T), max(linelist_sf$lon, na.rm=T)),   # limits of basemap tile
  lowerRight = c(min(linelist_sf$lat, na.rm=T), min(linelist_sf$lon, na.rm=T)),
  zoom = NULL,
  type = "osm")
...
ggplot() + 
  geom_sf(data = shapefile) +
  geom_sf(data = linelist_sf, aes(colour = fct_infreq(eth_race)), alpha = 0.5)


## if you didnt have your gps points in an sf object you could just use geom_point()
...
```{r choropleth_map_over}

shapefile <- shapefile %>% 
  mutate(cases = lengths(st_intersects(., linelist_sf)), 
         deaths = lengths(st_intersects(., 
                                        filter(linelist_sf, 
                                               died_covid == "Yes")))
  )
...
shapefile <- shapefile %>% 
  mutate(
    ...
    incidence = case_when(
           # fix the outliers: set infity to NA and cases less than 10 to NA
           cases < 10             ~ NA_real_,
           is.infinite(incidence) ~ NA_real_, 
           TRUE                   ~ incidence)
    )
...
shapefile <- shapefile %>% 
  mutate(
    # create a categorical variable using the cut function 
    incidence_cats = cut(incidence, breaks = c(0, breakers), 
                         labels = str_glue("<={breakers}"))
  )

ggplot(shapefile) + 
  geom_sf(aes(fill = incidence_cats), colour = "black") + 
  scale_fill_brewer(palette = "YlGn", 
                    labels = c(levels(shapefile$incidence_cats), "Fewer than 10 cases")) + 
  geom_sf_text(aes(label = ZipCode), check_overlap = TRUE) + 
...
xmin <- st_point(c(bounding_box[1], bounding_box[2]))
xmax <- st_point(c(bounding_box[3], bounding_box[2]))
ymin <- xmin
ymax <- st_point(c(bounding_box[1], bounding_box[4]))
...
xdist <- st_distance(xaxis)[2] %>% units::set_units("km")
ydist <- st_distance(yaxis)[2] %>% units::set_units("km")
...
ggplot() + 
  ...
  ggspatial::annotation_scale(unit_category = "metric") + 
  theme(
    panel.grid = element_blank()
  ) + 
  geom_vline(xintercept=seq(-84.8, -84.1, by=xfact)) + 
  geom_hline(yintercept=seq(33.5, 34.2, by=yfact)) + 
  geom_point(
        data = filter(linelist,
                      lat > 1 &
                        date_report %in% recent_period),
