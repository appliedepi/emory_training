...
shapefile <- shapefile %>% 
  mutate(
    incidence = case_when(
      recent        < 10     ~ NA_character_, 
      recent_inc   <= 18.6  ~ "≤18.6", 
      recent_inc   <= 21.3  ~ "≤21.3", 
      recent_inc   <= 22.7  ~ "≤22.7",
      recent_inc   <= 34.2  ~ "≤34.2", 
      TRUE                  ~ NA_character_
    ))
...
## TODO need to pull this out so age_categories works 
...
shapefile <- shapefile %>% 
  mutate(
    ...
    incidence_cats = age_categories(incidence, breakers = breakers)
  )
